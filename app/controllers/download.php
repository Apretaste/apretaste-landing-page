...
class Download 
{
	...
	public function main()
	{
		...
		// check if accesing from the .org or not
		$arrDomain = explode('.', $_SERVER['SERVER_NAME']);
		$directApkDownload = (end($arrDomain) == 'org') ? 0 : 1;
		...
		$this->view->data->directApkDownload = $directApkDownload;
		$this->view->data->ios = getDownloadLink('ios');
		$this->view->data->android = getDownloadLink('android');
		$this->view->data->apk = getDownloadLink('apk');
